...
def base_config() -> ConfigDict:
    ...
    # Alchemy: keep NO_OP transitions in the buffer (the critic still has to
    # value them) but leave them out of MATE's running mean. Silently disabled,
    # with the reason printed, wherever it does not apply -- other envs, models
    # that do not average, and STORE.
    config.memory_skip_no_op = True
